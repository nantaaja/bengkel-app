...
import { FiHelpCircle, FiLogOut, FiSettings, FiMenu } from "react-icons/fi";
  ...
  return (
    <header className="flex h-[70px] sm:h-[90px] shrink-0 items-center justify-between border-b border-white/10 px-3 sm:px-8">
      ...
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        ...
        <span className="hidden sm:inline text-lg text-zinc-400 shrink-0">Pemilik</span>
        ...
        {pathnames.length === 0 ? (
          ...
          pathnames.map((val, i) => (
            <div key={i} className="flex items-center gap-2 sm:gap-3 min-w-0">
              <span className="hidden sm:inline text-zinc-600">&gt;</span>
              <h1 className="text-base sm:text-lg font-semibold text-white truncate">{format(val)}</h1>
            </div>
          ))
        )}
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        <div className="relative ml-1 sm:ml-2" ref={dropdownRef}>
          ...
          {isOpen && (
            <div className="absolute right-0 mt-3 w-64 sm:w-72 origin-top-right rounded-2xl border border-white/10 bg-zinc-900 p-5 shadow-2xl z-50">
              ...
              <button
                ...
                className="flex w-full mb-2 items-center justify-center gap-2 rounded-xl border border-white/10 py-2.5 text-sm font-semibold text-zinc-300 transition-all hover:bg-white/10 hover:text-white"
              >
            ...
            <form onSubmit={handleSaveSettings} className="space-y-4">
              ...
              <div>
                ...
                <input type="email" required value={formData.email}
                  ...
                  className="w-full rounded-xl border border-white/10 bg-black/50 p-3 text-white outline-none focus:border-[#F24822]" />
                <p className="mt-2 text-xs text-red-400">*Mengubah email akan mengharuskan login ulang.</p>
              </div>
